...
class lstm_cell(object):
    ...
    def __init__(self, input_size, hidden_layer_size, target_size, time_steps):
        ...
        with tf.name_scope('rnn_weights_It'):
            ...
            self.Wc_i = tf.Variable(tf.zeros([self.hidden_layer_size, self.hidden_layer_size]))
        ...
        with tf.name_scope('rnn_weights_Ft'):
            ...
            self.Wc_f = tf.Variable(tf.zeros([self.hidden_layer_size, self.hidden_layer_size]))
        ...
        with tf.name_scope('rnn_weights_Ot'):
            ...
            self.Wc_o = tf.Variable(tf.zeros([self.hidden_layer_size, self.hidden_layer_size]))
        ...
        with tf.name_scope('rnn_weights_Gt'):
            ...
            self.Wc_g = tf.Variable(tf.zeros([self.hidden_layer_size, self.hidden_layer_size]))
    ...
    def lstm(self, previous_state, x):
        ...
        It = tf.sigmoid(tf.add_n([tf.matmul(x, self.Wx_i),tf.matmul(self.Wh_i, previous_hidden_state), tf.matmul(self.Wc_i, previous_c_state)]))
        Ft = tf.sigmoid(tf.add_n([tf.matmul(x, self.Wx_f),tf.matmul(self.Wh_f, previous_hidden_state), tf.matmul(self.Wc_f, previous_c_state)]))
        ...
        Ot = tf.sigmoid(tf.add_n([tf.matmul(x, self.Wx_o),tf.matmul(self.Wh_o, previous_hidden_state), tf.matmul(self.Wc_o, Ct)]))
