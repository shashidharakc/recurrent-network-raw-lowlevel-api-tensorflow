...
class LSTM_Vanila_build(tf.keras.layers.Layer):
    """
    This is a class for computing LSTM, as described in colah's blog.
    ...
    def __init__(self, BATCH_SIZE, HIDDEN_UNITS, VOCAB_SIZE):
        ...
        super(LSTM_Vanila_build, self).__init__()
    ...
    def call(self, input_data, previous_hidden_state,previous_c_state):
        """Define forward pass"""
        Ft = tf.sigmoid(tf.matmul(input_data, self.w_ip_f) + tf.matmul(previous_hidden_state, self.w_forget))
        It = tf.sigmoid(tf.matmul(input_data, self.w_ip_i) + tf.matmul(previous_hidden_state, self.w_input))
        ...
        Ot = tf.sigmoid(tf.matmul(input_data, self.w_ip_o) + tf.matmul(previous_hidden_state, self.w_output))
